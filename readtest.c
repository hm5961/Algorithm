...
//// 트리로 사용 할 노드
//
//void make_clean_tree(Node** h); // 트리의 헤드를 생성하는 함수
//void insert_node(Node* h, int tmp); // tmp를 키값으로 가지는 노드를 트리에 삽입하는 함수
...
//    Node* head = NULL; // 헤드노드
//    make_clean_tree(&head); // 헤드노드 트리형태로 초기화
//
//    // 파일 읽기 성공
//    while (1)
//    {
...
//            break;
//        insert_node(head, intTemp); // 트리에 intTemp삽입
//    }
...
//
//    if (h->left == NULL) // 루트노드가 없을 경우 n을 루트노드로 지정
//    {
//        h->left = n;
//        return;
//    }
...
//                p = p->left;
//        }
//        else if (n->key == p->key) // 이미 같은 값으로 저장된 값이 존재할 경우 오류 메세지 출력 후 저장하지 않고 종료
//        {
//            //printf("중복된 값이 존재합니다.\n");
//            return;
//        }
