...
void makecleantree(Node* n, Node* p, int height) // 조건부 삭제
{
    ...
    if (n->left || n->right) {
        ...
        { // n의 높이가 정상범위이며 n->alpha값이 영 대/소문자 범위일 경우
            ...
            printf("nonzero = %d \t roop = %d \t freq = %d \t alpha = %d\n", height, roop, n->freq, n->alpha);
            if (n->freq == n->left->freq)
            {
                printf("calhigh = % d \t n->alpha = % d \t n->freq = % d\n", height, n->alpha, n->freq);
                printf("b");
                if (p->left == n)
                {
                    printf("-1\n");
            ...
            {
                printf("cal height = %d \t n->alpha = %d \t n->freq = %d\n", height, n->alpha, n->freq);
                printf("c");
                if (p->left == n)
                {
                    printf("-1\n");
...
void recursiveInorderbin(Node* ptr)
{
    if (ptr) { // ptr의 자식노드로 재귀된 값이 노드의 형태 일 때 
        ...
        if(!ptr->left && !ptr->right)
            printf("[%c = %s]", ptr->alpha, ptr->bin); // 탐색된 값 출력 
